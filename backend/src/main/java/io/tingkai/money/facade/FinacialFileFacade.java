...
public class FinacialFileFacade {
	...
	public List<FinancialFile> queryAll(UUID userId) {
		...
		if (entities.size() == 0) {
			log.trace(MessageFormat.format(MessageConstant.QUERY_NO_DATA, DatabaseConstants.TABLE_FINANCIAL_FILE));
		}
		return entities;
	}
	...
	public FinancialFile insert(FinancialFile entity) throws AlreadyExistException, FieldMissingException {
		if (!AppUtil.isAllPresent(entity, entity.getUserId(), entity.getFilename(), entity.getType(), entity.getDate())) {
	...
	public FinancialFile update(FinancialFile entity) throws NotExistException, FieldMissingException {
		if (!AppUtil.isAllPresent(entity, entity.getId(), entity.getUserId(), entity.getFilename(), entity.getType(), entity.getDate())) {
			throw new FieldMissingException();
		}
